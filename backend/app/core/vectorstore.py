...
def _client():
    ...
    return chromadb.PersistentClient(path=str(settings.chroma_dir), settings=Settings(
        anonymized_telemetry=False,
        chroma_product_telemetry_impl="app.core.telemetry.DisabledTelemetry",
        chroma_telemetry_impl="app.core.telemetry.DisabledTelemetry",
    ))
...
def add_document_chunks(
    *, document_id: str, filename: str, category: str, chunks: list[Chunk]
    ...
    previous_ids = collection.get(where={"document_id": document_id}, include=[])["ids"]
    collection.upsert(ids=ids, documents=texts, embeddings=vectors, metadatas=metadatas)
    obsolete = list(set(previous_ids) - set(ids))
    if obsolete:
        collection.delete(ids=obsolete)
    from app.core.bm25 import _index_for_count
    _index_for_count.cache_clear()
