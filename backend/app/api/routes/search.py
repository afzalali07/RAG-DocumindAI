...
from app.core.extraction import location_kind
...
def vector_search(
    ...
    ids = [s.strip() for s in (document_ids or "").split(",") if s.strip()]
    if document_ids is not None and not ids:
        raise HTTPException(422, "Select a document before searching.")
    ...
    for h in hits:
        ...
        results.append(
            Source(
                ...
                label=h.get("label"),
                location_kind=location_kind(h["filename"]),
            )
        )
