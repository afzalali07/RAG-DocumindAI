...
async def chat(
    ...
    req.model = configured_model_id()
    if mode not in ("rag", "agent", "compare"):
        mode = "rag"

    comparison = None
    if req.document_ids is not None and mode != "compare":
        if not req.document_ids:
            raise HTTPException(422, "Select a document before continuing.")
        for document_id in req.document_ids:
            document = db.get(Document, document_id)
            if document is None:
                raise HTTPException(404, "The selected document no longer exists.")
            if document.status != "ready":
                raise HTTPException(409, "The selected document is not ready.")
    if mode == "compare":
        ids = req.document_ids or []
        if len(ids) != 2 or len(set(ids)) != 2:
            raise HTTPException(422, "Choose exactly two different documents.")
        docs = [db.get(Document, document_id) for document_id in ids]
        if any(doc is None for doc in docs):
            raise HTTPException(404, "A selected document no longer exists.")
        if any(doc.status != "ready" for doc in docs):
            raise HTTPException(409, "Both documents must finish processing before comparison.")
        try:
            comparison = await run_in_threadpool(prepare_comparison, docs, req.message, req.lang)
    ...
    if conv is not None and set(conv.document_ids or []) != set(req.document_ids or []):
        raise HTTPException(409, "Start a new conversation when changing documents.")
    conv_created = conv is None
    if conv is None:
        conv = Conversation(title=_title_from(req.message), model=req.model, document_ids=req.document_ids)
        db.add(conv)
        db.commit()
        db.refresh(conv)
    ...
    hits = comparison[0] if comparison else rag.retrieve(
        query=req.message,
        top_k=req.top_k,
    ...
    system = comparison[1] if comparison else rag.build_system_prompt(hits, req.lang)
    followups = [] if comparison else _static_followups([s.filename for s in sources], req.lang)
    if comparison:
        # Prior conversation messages must not leak a different pair into a comparison.
    ...
    async def gen_rag() -> AsyncIterator[str]:
        ...
        buffer: list[str] = []
        try:
            if comparison and provider.provider == "mock":
                buffer.append(comparison[2])
                yield _sse("token", {"delta": comparison[2]})
            else:
