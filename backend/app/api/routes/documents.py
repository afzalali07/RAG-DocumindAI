...
from app.core.extraction import cache_path, get_extraction
...
def inspect_document(document_id: str, db: Session = Depends(get_db)):
    ...
    if doc.status != "ready":
        raise HTTPException(409, "Wait until the document has finished processing.")
    try:
        result = get_extraction(doc)
    except FileNotFoundError as exc:
        raise HTTPException(404, str(exc)) from exc
    except ParseError as exc:
        raise HTTPException(422, str(exc)) from exc
    # Upgrade page counts for documents indexed before blank pages were retained.
    if doc.page_count != result["page_count"]:
...
def reindex_document(document_id: str, background: BackgroundTasks, db: Session = Depends(get_db)):
    doc = db.get(Document, document_id)
    if not doc:
        raise HTTPException(404, "Document not found")
    if doc.status == "processing":
        raise HTTPException(409, "Document is already processing")
    path = settings.upload_dir / f"{doc.id}{Path(doc.filename).suffix.lower()}"
    if not path.is_file():
    ...
    db.commit()
    db.refresh(doc)
    background.add_task(_run_ingestion, doc.id, str(path))
    return doc
...
def delete_document(document_id: str, db: Session = Depends(get_db)):
    ...
    for doc_id in ids:
        ...
        cache_path(doc_id).unlink(missing_ok=True)
        db.delete(target)
    db.commit()
