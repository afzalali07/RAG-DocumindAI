...
- 🤖 **Local Llama models** — backend configuration selects Llama 3.2 3B or Llama 3.1 8B.
...
The app uses local Ollama inference and local embeddings. Configure `OLLAMA_MODEL`
in `backend/.env` before starting; see **Backend model configuration** below.
...
cp .env.example .env         # choose OLLAMA_MODEL before starting
...
Compose loads the model setting from `backend/.env` and connects to Ollama on the
host via `host.docker.internal:11434`.
...
## Choose a document before chatting

In RAG Chat, AI Agent, and Vector Search, use **Selected document** to choose one
uploaded file. The input becomes available only after a ready file is selected.
Questions, agent tools, suggestions, and search results are restricted to that file.
Changing files starts a new conversation; existing history stays saved. Reopening
a scoped conversation restores its document. **New chat** asks you to select again.
If a selected document is deleted, select another file before continuing.

Comparison mode still uses its own two-document selection. Categories remain
available for organizing uploads; the selected file determines retrieval scope.

## Tables and document comparison

Choose **Mode → Compare documents**, then **Upload two files**, or select two
existing documents. Wait for both to finish processing,
optionally enter a comparison focus, and click **Compare documents**.

The comparison includes up to three 400-token excerpts from each document and
cites the source file and location. It is an excerpt-based comparison, not a full
document diff. The backend-configured Llama model performs analysis.
Answers and citations are saved in conversation history.

Use **Tables & pages** on a document to browse extracted tables and text, move
between locations, and open the original. Existing uploads are extracted on first
inspection/comparison, so these features do not require re-uploading.

- PDF locations are physical pages, including blank pages.
- Word locations are logical sections, not printed pages.
- Excel locations are worksheets. Empty cells retain their column positions.
- PDF table detection uses cell borders; borderless tables may be missed. Scanned
  PDFs need OCR, which is not included.
- Excel extraction covers the first 2,000 rows and 100 columns per sheet and warns
  when truncated. Formula cells use cached values saved in the workbook.
- Newly uploaded tables are searchable. Existing uploads keep their old search
  index; re-upload them to use improved table text in ordinary RAG/vector search.
  Inspection and comparison always use the new extraction.

Install the updated `backend/requirements.txt` and restart the backend. Continue
using `npm run dev` for the frontend. Docker users should rebuild their containers.

API: `GET /api/documents/{id}/extraction` returns locations, text, table rows, and
warnings. `POST /api/chat` accepts `mode: "compare"` and exactly two distinct ready
document IDs in `document_ids`.

## Backend model configuration

There is no model selector in the frontend. RAG Chat, AI Agent, and document
comparison all use the backend setting. Only two chat models are supported:

```dotenv
# backend/.env — select ONE value
OLLAMA_MODEL=llama3.2:3b
# OLLAMA_MODEL=llama3.1:8b
OLLAMA_BASE_URL=http://localhost:11434
```

Download the chosen model before starting:
...
Restart the backend after changing `.env`. Invalid model names fail validation at
startup. Client-supplied model values and old conversation model selections cannot
override the backend setting. Ollama errors are shown instead of falling back to
another model. Existing `llama3.1:latest` downloads are accepted as the default 8B
variant. Vector Search uses embeddings and does not require the chat model.

For Docker Compose, create `backend/.env` from `.env.example`, set `OLLAMA_MODEL`,
then recreate the backend container. The existing Compose host URL stays in use.
For a standalone Docker image, pass `--env-file backend/.env` at runtime and set
`OLLAMA_BASE_URL` to a reachable Ollama host; do not bake configuration into the image.
...
- **LLM:** Ollama, configured as Llama 3.2 3B or Llama 3.1 8B in the backend
