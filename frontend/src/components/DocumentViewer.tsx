  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) {
          setSheets(parsed)
          setActive(Math.max(0, Math.min((trackParamsRef.current.page ?? 1) - 1, parsed.length - 1)))
          setStatus('ready')
        }
      } catch (e) {
        if (!cancelled) {
      ...
      <div className="viewer" onClick={(e) => e.stopPropagation()}>
        <div className="viewer-head">
          <div className="viewer-title">
            ...
            {source.page != null && (
              <span className="viewer-page">{source.label || (mode === 'pdf' ? t('page', { n: source.page }) : `${mode === 'xlsx' ? 'Sheet' : 'Section'} ${source.page}`)}</span>
            )}
          </div>
