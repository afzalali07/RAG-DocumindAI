...
import type { ChatMode, Conversation, DocumentItem, Source } from './lib/types'

type Theme = 'light' | 'dark'
const THEMES: readonly Theme[] = ['light', 'dark']
const MODES: readonly ChatMode[] = ['rag', 'agent', 'search', 'compare']
...
export default function App() {
  ...
  const handleSend = useCallback(
    ...
    [send, category, lang, mode, selectedDocument],
  )
  ...
  const handleUploadDocuments = () => {
    // Mount the panel and dismiss the modal before opening its file input.
    // Keep the click in this user gesture so browsers allow the native picker.
    flushSync(() => {
  ...
  const handleOpenSource = useCallback((source: Source) => {
    ...
    if (source.page && !source.filename.toLowerCase().endsWith('.pdf')) {
      setInspection({ id: source.document_id, page: source.page })
    } else {
